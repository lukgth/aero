...
function Ensure-WdkMSBuildShim {
  # Creates the minimal WDK MSBuild integration files that are normally shipped with the
  # WDK VS extension but may be absent on CI runners where the WDK tools are installed
  # without the full VS extension (e.g. windows-latest with VS 2025).
  #
  # Returns a hashtable:
  #   PropsCreated    - $true if WindowsDriver.common.props was created (or already existed)
  #   TargetsPath     - absolute path to the generated kernel-driver targets shim (or $null)
  #   WdkBuildDir     - the WDK build\ directory (may not exist before this function runs)
  param(
    [Parameter(Mandatory = $true)][string]$WdkContentRoot,
    [string]$KmLibDir = $null,   # Versioned lib\<ver>\km\<arch> *parent* — used in targets
    [string]$OutDir = $null      # Directory to write the targets shim into
  )
  ...
  # --- 1. WindowsDriver.common.props shim ---
  # This file must live at $(WDKContentRoot)build\WindowsDriver.common.props so that the
  # WindowsKernelModeDriver10.0 Toolset.props conditional import picks it up.
  $buildDir = Join-Path $WdkContentRoot 'build'
  $result.WdkBuildDir = $buildDir

  $commonPropsPath = Join-Path $buildDir 'WindowsDriver.common.props'
  if (-not (Test-Path -LiteralPath $commonPropsPath)) {
    Write-Host "WDK MSBuild shim: creating $commonPropsPath"
    if (-not (Test-Path -LiteralPath $buildDir)) {
      New-Item -ItemType Directory -Path $buildDir -Force | Out-Null
    }

    # Per-platform props files define the architecture preprocessor macro required by ntdef.h.
    # Without _X86_=1 (Win32) or _AMD64_=1 (x64), ntdef.h emits C1189 "No Target Architecture".
    $win32PropsPath = Join-Path $buildDir 'WindowsDriver.Win32.props'
    $x64PropsPath   = Join-Path $buildDir 'WindowsDriver.x64.props'
...
<Project xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <ItemDefinitionGroup>
    <ClCompile>
      <PreprocessorDefinitions>_X86_=1;%(PreprocessorDefinitions)</PreprocessorDefinitions>
    </ClCompile>
  </ItemDefinitionGroup>
</Project>
'@
...
<Project xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <ItemDefinitionGroup>
    <ClCompile>
      <PreprocessorDefinitions>_AMD64_=1;%(PreprocessorDefinitions)</PreprocessorDefinitions>
    </ClCompile>
  </ItemDefinitionGroup>
</Project>
'@
...
<?xml version="1.0" encoding="utf-8"?>
<!--
  Minimal WindowsDriver.common.props shim generated by ci/build-drivers.ps1.
  Normally shipped with the WDK VS extension; created here because that extension
  is not present on this runner.
-->
<Project xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <Import Project="$(MSBuildThisFileDirectory)WindowsDriver.Win32.props"
          Condition="'$(Platform)'=='Win32'" />
  <Import Project="$(MSBuildThisFileDirectory)WindowsDriver.x64.props"
          Condition="'$(Platform)'=='x64'" />
</Project>
'@

    $utf8NoBom = [System.Text.UTF8Encoding]::new($false)
    [System.IO.File]::WriteAllText($win32PropsPath, $win32PropsContent, $utf8NoBom)
    [System.IO.File]::WriteAllText($x64PropsPath,   $x64PropsContent,   $utf8NoBom)
    [System.IO.File]::WriteAllText($commonPropsPath, $commonPropsContent, $utf8NoBom)
    Write-Host "WDK MSBuild shim: wrote WindowsDriver.common.props + platform props"
    $result.PropsCreated = $true
  } else {
    Write-Host "WDK MSBuild shim: WindowsDriver.common.props already exists — skipping creation"
    $result.PropsCreated = $true
  }

  # --- 2. Kernel-driver targets shim (ForceImportAfterCppTargets) ---
  # This runs after Microsoft.Cpp.targets and applies kernel-mode linker settings that the
  # full WDK toolset would normally inject via WindowsDriver.common.targets.
  # Required because ConfigurationType=Driver + WindowsKernelModeDriver10.0 without the
  # WDK VS extension defaults to user-mode linker settings (/SUBSYSTEM:WINDOWS, no /DRIVER:WDM).
  if (-not $OutDir) { return $result }

  if (-not (Test-Path -LiteralPath $OutDir)) {
    New-Item -ItemType Directory -Path $OutDir -Force | Out-Null
  }

  $targetsPath = Join-Path $OutDir 'wdk-kmd-shim.targets'

  # Build library search path and dependency entries only when we have km lib dirs.
  # $KmLibDir is the versioned km parent (e.g. C:\...\Lib\10.0.26100.0\km), and
  # the platform-specific sub-dirs (x86 / x64) are resolved at evaluation time.
  $libDirSnippet = ''
  $libDepSnippet = 'ntoskrnl.lib;hal.lib;%(AdditionalDependencies)'
  if (-not [string]::IsNullOrWhiteSpace($KmLibDir)) {
    # Embed the km root as an absolute path; MSBuild will append the arch sub-dir.
    $kmLibDirEscaped = $KmLibDir.TrimEnd('\', '/')
    $libDirSnippet = @"
      <AdditionalLibraryDirectories Condition="'`$(Platform)'=='Win32'">$kmLibDirEscaped\x86;%(AdditionalLibraryDirectories)</AdditionalLibraryDirectories>
      <AdditionalLibraryDirectories Condition="'`$(Platform)'=='x64'">$kmLibDirEscaped\x64;%(AdditionalLibraryDirectories)</AdditionalLibraryDirectories>
"@
  }
...
<?xml version="1.0" encoding="utf-8"?>
<!--
  Kernel-mode driver linker/compiler shim generated by ci/build-drivers.ps1.
  Imported via ForceImportAfterCppTargets so it runs after Microsoft.Cpp.targets.

  IMPORTANT: This file is the authoritative source for architecture preprocessor
  defines (_X86_=1 / _AMD64_=1) on runners where the WDK VS extension is absent.
  The WindowsDriver.common.props in the WDK build\ dir may not be imported by the
  VS 2025 WindowsKernelModeDriver10.0 Toolset.props, so we inject the defines here
  via an explicit ForceImportAfterCppTargets rather than relying on that chain.
-->
<Project xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <!--
    Architecture preprocessor defines required by ntdef.h (line 197):
      #error "No Target Architecture" if none of _X86_/_AMD64_/_IA64_/_ARM_/_ARM64_ are defined.
    These are normally set by WindowsDriver.common.props -> WindowsDriver.{Win32,x64}.props,
    but that import chain may be broken on VS 2025. Emit them unconditionally here for
    WindowsKernelModeDriver10.0 projects — they are idempotent if already defined.
  -->
  <ItemDefinitionGroup Condition="'`$(PlatformToolset)'=='WindowsKernelModeDriver10.0' and '`$(Platform)'=='Win32'">
    <ClCompile>
      <PreprocessorDefinitions>_X86_=1;%(PreprocessorDefinitions)</PreprocessorDefinitions>
    </ClCompile>
  </ItemDefinitionGroup>
  <ItemDefinitionGroup Condition="'`$(PlatformToolset)'=='WindowsKernelModeDriver10.0' and '`$(Platform)'=='x64'">
    <ClCompile>
      <PreprocessorDefinitions>_AMD64_=1;%(PreprocessorDefinitions)</PreprocessorDefinitions>
    </ClCompile>
  </ItemDefinitionGroup>

  <!-- Kernel-mode compiler + linker settings for WindowsKernelModeDriver10.0 projects. -->
  <ItemDefinitionGroup Condition="'`$(PlatformToolset)'=='WindowsKernelModeDriver10.0'">
    <ClCompile>
      <!-- Kernel mode: disable exceptions and buffer security checks. -->
      <ExceptionHandling>false</ExceptionHandling>
      <BufferSecurityCheck>false</BufferSecurityCheck>
      <!-- /kernel enables kernel-safe code generation (SEH, function pointers). -->
      <AdditionalOptions>/kernel %(AdditionalOptions)</AdditionalOptions>
    </ClCompile>
    <Link>
      <!-- Native subsystem + WDM driver mode; entry point is DriverEntry. -->
      <SubSystem>Native</SubSystem>
      <EntryPointSymbol>DriverEntry</EntryPointSymbol>
      <AdditionalOptions>/DRIVER:WDM %(AdditionalOptions)</AdditionalOptions>
      <!-- Suppress all default libs (CRT etc.); kernel code must not call CRT functions. -->
      <IgnoreAllDefaultLibraries>true</IgnoreAllDefaultLibraries>
$libDirSnippet
      <AdditionalDependencies>$libDepSnippet</AdditionalDependencies>
    </Link>
  </ItemDefinitionGroup>

  <!-- Force .sys output extension for Driver configuration type. -->
  <PropertyGroup Condition="'`$(PlatformToolset)'=='WindowsKernelModeDriver10.0'">
...
"@
  ...
  Write-Host "WDK MSBuild shim: wrote kernel-driver targets shim: $targetsPath"
  $result.TargetsPath = $targetsPath
  return $result
}
...
#
# If WindowsDriver.common.props is missing we generate a minimal shim directly into the WDK
# build\ directory. A kernel-driver targets shim is also generated and injected via
# ForceImportAfterCppTargets to supply the kernel-mode linker settings that the full WDK VS
...
# Resolve the versioned WDK km lib directory (e.g. C:\...\Lib\10.0.26100.0\km) so the
# targets shim can point the linker at ntoskrnl.lib / hal.lib.
$wdkKmLibDir = $null
if ($null -ne $wdkContentRoot) {
  $libRoot = Join-Path $wdkContentRoot 'Lib'
  if (Test-Path -LiteralPath $libRoot) {
    $verDirs = @(
      Get-ChildItem -LiteralPath $libRoot -Directory -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -match '^\d+\.\d+\.\d+\.\d+$' } |
        Sort-Object { [Version]$_.Name } -Descending
    )
    foreach ($vd in $verDirs) {
      $kmDir = Join-Path $vd.FullName 'km'
      if (Test-Path -LiteralPath $kmDir) {
        $wdkKmLibDir = $kmDir
        Write-Host "Resolved WDK km lib dir: $wdkKmLibDir"
        break
      }
    }
  }
  if ($null -eq $wdkKmLibDir) {
    Write-Host "Warning: WDK km lib directory not found under $libRoot; ntoskrnl.lib / hal.lib will not be added automatically."
  }
}

# Ensure WDK MSBuild shim files exist. This is a no-op when the WDK VS extension is properly
# installed (WindowsDriver.common.props already present); on bare CI runners it creates the
# minimal props/targets needed for the driver build to succeed.
if ($null -ne $wdkContentRoot) {
  $shimOutDir = Join-Path $outRoot 'wdk-shim'
  $shimResult = Ensure-WdkMSBuildShim `
    -WdkContentRoot $wdkContentRoot `
    -KmLibDir $wdkKmLibDir `
    -OutDir $shimOutDir

  if ($null -ne $shimResult.TargetsPath) {
    # ForceImportAfterCppTargets injects our targets file after Microsoft.Cpp.targets runs,
    # so our kernel-mode linker settings take precedence over the user-mode defaults.
    $globalExtraMSBuildProps += "/p:ForceImportAfterCppTargets=$($shimResult.TargetsPath)"
    Write-Host "WDK MSBuild shim: injecting kernel-driver targets via ForceImportAfterCppTargets"
  }
} else {
  Write-Host "Warning: WDK content root not found; MSBuild shim will not be created."
}
