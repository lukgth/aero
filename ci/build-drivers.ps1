...
<?xml version="1.0" encoding="utf-8"?>
<!--
  Kernel-mode driver linker shim generated by ci/build-drivers.ps1.
  Imported via ForceImportAfterCppTargets so it runs after Microsoft.Cpp.targets
  and takes precedence over user-mode linker defaults.
-->
<Project xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <!--
    Apply only to WindowsKernelModeDriver10.0 projects to avoid breaking UMD / test projects
    in the same solution that use a standard toolset (v143 etc.).
  -->
  <ItemDefinitionGroup Condition="'`$(PlatformToolset)'=='WindowsKernelModeDriver10.0'">
    <ClCompile>
      <!-- Kernel mode: disable exceptions, buffer security checks, and CRT runtime selection. -->
